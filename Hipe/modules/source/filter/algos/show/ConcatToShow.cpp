...
cv::Mat filter::algos::ConcatToShow::ShowManyImages(std::vector<data::ImageData> arrayData) const
{
	...
	for (int h_cur = 0; h_cur < h; h_cur++)
	{
		...
		{
			cv::Mat currentMat = arrayData[imgIdx].getMat();
			int x = currentMat.size().width;
			int y = currentMat.size().height;
			...
			cv::Mat temp;
			cv::resize(currentMat, temp, cv::Size(ROI.width, ROI.height));
