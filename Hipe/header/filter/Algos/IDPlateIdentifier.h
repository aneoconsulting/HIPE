...
namespace filter {
	namespace Algos {
		...
		private:
			std::vector<cv::Mat> detectTextArea(const cv::Mat & plateImage);
		...
		public:
			...
			std::vector<std::string> runRecognition(const std::vector<cv::Mat>& labelImage, int labelType);
		...
		ADD_CLASS(IDPlateIdentifier, unused);
	}
}
