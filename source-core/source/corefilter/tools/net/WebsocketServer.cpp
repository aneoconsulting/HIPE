...
context_ptr on_tls_init2(tmode mode, std::string basename, websocketpp::connection_hdl hdl)
{
...
WebsocketServer* ServerInit(std::function<void(WebsocketServer*, websocketpp::connection_hdl)> con_callback,
  ...
  websocket_server->set_tls_init_handler(bind(&on_tls_init2,MOZILLA_INTERMEDIATE, basename, ::_1));
