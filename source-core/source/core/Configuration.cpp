...
namespace hipe_server
{
	...
	void ConfigurationParameters::setDefaultValues()
	{
		...
		this->aschildproc = "NOT-DEFINED";
		this->json_request_file = "NOT-DEFINED";
	...
	int Configuration::setConfigFromCommandLine(int argc, char* argv[])
	{
		...
		configCat.add_options()
			...
			;
		configCat.add_options()
			("shared,s", bpo::value<std::string>(&this->configuration.aschildproc)->default_value(""), "(Engine Only) Sets the engine has child of one server. Name of shared memory")
			;
		configCat.add_options()
			("json_file,f", bpo::value<std::string>(&this->configuration.json_request_file)->default_value(""), "(Engine Only) Set a json request if the process is standalone")
			;
