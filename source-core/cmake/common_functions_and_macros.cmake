...
macro(install_header_to_target targetname)
	...
	foreach ( file ${${targetname}_header} )
		...
		message(STATUS "copy file : ${CMAKE_SOURCE_DIR}/header/${targetname}/${file} ---> ${CMAKE_BINARY_DIR}/temp/${file}" )
		
		CONFIGURE_FILE("${CMAKE_SOURCE_DIR}/header/${targetname}/${file}" "${CMAKE_BINARY_DIR}/temp/${file}" @ONLY)
		FILE(MD5 "${CMAKE_BINARY_DIR}/temp/${file}" new_file_with_header)
		...
		if (NOT "${old_file_with_header}" STREQUAL ${new_file_with_header})
			message(STATUS "CHECKSUM HEADER FILE DIFFER : [${CMAKE_INSTALL_PREFIX}/include/${targetname}/${dir}/${filename}] <--> [${CMAKE_BINARY_DIR}/temp/${file}]")
			message(STATUS "CHECKSUM HEADER FILE DIFFER : [${old_file_with_header}] <--> [${new_file_with_header}]")
			install( FILES "${CMAKE_BINARY_DIR}/temp/${file}" DESTINATION include/${targetname}/${dir}  COMPONENT headers )
		else()
...
macro(install_dependencies_int target_name EXT_BIN)
	...
	install(CODE "set(target_name \"${target_name}\")" COMPONENT applications)
	install(CODE "set(EXT_BIN \"${EXT_BIN}\")" COMPONENT applications)
	install(CODE "set(HIPE_EXTERNAL_DIR \"${HIPE_EXTERNAL_DIR}\")" COMPONENT applications)
	install(CODE "set(PROJECT_SOURCE_DIR \"${PROJECT_SOURCE_DIR}\")" COMPONENT applications)
