    ...
    $("#board").droppable({
        ...
        drop: function (event, ui) {
            ...
            //if (isDataSource($(ui.draggable).attr('id'))) {
                ...
                $('#button_collapse_' + (idNode)).html('<span class="glyphicon glyphicon-chevron-down"></span> ' + type + ' (' + nodes[element].data.name + ')');
        ...
        success: function (data) {
            Object.keys(data).forEach(function (el) {
                if (el != "Version" && el != "Hash" && el != "Filters" && el != "GroupFilters") {
                    ...
                    $('#command').append('<li><a href="javascript:void(0)" onclick="sendCommand(\'' + el + '\')">' + el + '</a></li>');
                }
            })
