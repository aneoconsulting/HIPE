...
function build_cuda10()
{
  ...
  local install_dir="/usr/local/cuda-8.0"
  add_paths "$install_dir"
...
#   fi
  ...
  then
    local cuda_install_dir="/usr/local/cuda-8.0"
...
#   maybe_remove_build_files "ceres-solver"
  ...
  then
    ...
    pushd "$BUILD_DIRECTORY/tbb-2018_U2"
      CXXFLAGS="${CXXFLAGS:-} -fno-lifetime-dse" make_with_gcc_x 7
  #     make_with_gcc_x 5
  ...
  untar "$ARCHIVE_DIRECTORY/opencv_contrib-3.4.3.zip"
  untar "$ARCHIVE_DIRECTORY/opencv-3.4.3.zip"

  # Disable detection of ceres-solver.
  sed -i 's/find_package(Ceres QUIET)/set(Ceres_FOUND FALSE)/' \
    "$BUILD_DIRECTORY/opencv_contrib-3.4.3/modules/sfm/CMakeLists.txt"

  # ensure clean build directory
  local build_dir="$BUILD_DIRECTORY/opencv-3.4.3/build"
#   rm -rf -- "$build_dir"
  mkdir -p -- "$build_dir"
  pushd -- "$build_dir"
  cmake_with_gcc_x 7 \
