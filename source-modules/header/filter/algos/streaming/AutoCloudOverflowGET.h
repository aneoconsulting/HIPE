...
namespace filter
{
	namespace algos
	{
		class AutoCloudOverflowGET : public filter::IFilter
		{
			SET_NAMESPACE("vision/Cloud")

			CONNECTOR(data::ImageData, data::ImageData);

			REGISTER(AutoCloudOverflowGET, ()), _connexData(data::INDATA)
			{
				_debug = 0;
				skip_frame = 0;
				id = -1;
		...
		ADD_CLASS(AutoCloudOverflowGET, _debug, skip_frame, dir_path, prefix_filename);
